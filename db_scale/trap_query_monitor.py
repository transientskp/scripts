...
from tkp import config
from tkp.db import database as tkpdb
...
def gen_dataset():
    dataset = DataSet(data = {'description': 'My Sim'})
    return dataset
...
def gen_image(db, ds, imgprops):
    ...
    image = Image(dataset=ds, data=imgprops)
...
def gen_xsources(kappa, bsrc):
    ...
    error_radius_deg = 180. / (pi * sqrt(kappa)) # in deg
    #error_radius = 648000. / (pi * sqrt(kappa)) # in arcsec
    ra_fit_err = alpha(error_radius_deg, rsrc['decl']) # in deg
    decl_fit_err = ra_fit_err
    error_radius = 3600. * error_radius_deg
...
def main():
    ...
    for i in range(nimages):
        ...
        for freq in freqs:
            ...
            logfile = open(logdir + '/_associate_extracted_sources.log', 'a')
            start = time.time()
            associate_extracted_sources(imgid, deRuiter_r=5.68)
