...
if len(sys.argv) != 6:
    print 'python train_TraP.py <precision threshold> <recall threshold> <lda> <anomaly> <logistic>'
    exit()
...
###### LOGISTIC REGRESSION #######
    ...
    # sort the data into a training, validation and testing dataset. This is hardcoded to be 50%, 25% and 25% (respectively) of the total dataset
    train, valid, test = train_logistic_regression.create_datasets(shuffled, len(shuffled)*0.5, len(shuffled)*0.75)
    ...
    train_logistic_regression.plotLC(error_train, error_valid, "learning", True, True, "Number")
    ...
    error_train, error_valid, theta = train_logistic_regression.validation_curve(Xtrain, ytrain.T, Xvalid, yvalid.T, options)
    train_logistic_regression.plotLC(error_train, error_valid, "validation", True, True, r"$\lambda$")
    ...
    for counter in range(1000):
        shuffled = train_logistic_regression.shuffle_datasets(data)
        train, valid, test = train_logistic_regression.create_datasets(shuffled, len(shuffled)*0.5, len(shuffled)*0.25)
        Xtrain, ytrain = train_logistic_regression.create_X_y_arrays(train)
        Xvalid, yvalid = train_logistic_regression.create_X_y_arrays(valid)
        Xtest, ytest = train_logistic_regression.create_X_y_arrays(test)
        ...
        error_train.append(train_logistic_regression.check_error(Xtrain,ytrain))
        error_valid.append(train_logistic_regression.check_error(Xvalid,yvalid))
        predictions = train_logistic_regression.sigmoid(Xtest * np.c_[theta])
    train_logistic_regression.plotLC(error_train, error_valid, "repeat", False, True, "Trial number")

    # classify the full dataset and check results
    print "Classifying full dataset"
    X, y = train_logistic_regression.create_X_y_arrays(shuffled,len(shuffled),0.)
    tp, fp, fn, tn, classified = train_logistic_regression.classify_data(X,y,theta)
